...
function KhuyenMai(props) {
  ...
  function closeDidalog() {
    const { fetchListKhuyenMaiRequest, setKhuyenMaiEdittingNull } = createKhuyenMai;
    api
      .deleteKhuyenMai(khuyenMaiEditting)
      .then((responseCTKM) => {
        if (responseCTKM.status === 200) {
          deleteKhuyenMai(khuyenMaiEditting);
           fetchListKhuyenMaiRequest();
  ...
  function deleteKM(data) {
    ...
    setShow(true);
    setNd(
      `Bạn muốn xóa ${data.ten_khuyen_mai} ra khỏi chương trình khuyến mãi`
    );
  }
              ...
              ? ListKhuyenMai.map((post, index) => {
                  return (
                    <tr key={index + 1}>
                      <td>{index + 1}</td>
                      <td>
                        {Moment(post.ngay_bat_dau)
                          .utc()
                          .format("DD-MM-YYYY hh:mm")}
                      </td>
                      <td>
                        {Moment(post.ngay_ket_thuc)
                          .utc()
                          .format("DD-MM-YYYY hh:mm")}
                      </td>
